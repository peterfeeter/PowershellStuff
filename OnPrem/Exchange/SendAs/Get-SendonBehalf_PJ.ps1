...
$mailboxes = Get-Mailbox lmyers | where {$_.grantsendonbehalfto -ne $null -and $_.distinguishedName -notlike "*newco migrated*"}
$count = 1
...
foreach ($mailbox in $mailboxes)
{
	...
	{
		$sendonbehalf = Get-Recipient $sendonbehalfto.DistinguishedName
		
		$obj = New-Object System.Management.Automation.PSObject
		$obj | Add-Member NoteProperty Name $mailbox.name
		$obj | Add-Member NoteProperty SMTP $mailbox.PrimarySMTPAddress
        $obj | Add-Member NoteProperty OU $mailbox.OrganizationalUnit
