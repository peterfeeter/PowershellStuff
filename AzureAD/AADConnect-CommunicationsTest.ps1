...
function OnlineEndPoints
{
	switch -regex ($OnlineEndPointTarget)
	{
		...
		'dod|gcchigh'
		{
			...
			$RequiredResources = @(
				...
				"login.windows.net",
				"graph.windows.net", # have not verified
	...
	{
		try { [array]$ResourceAddresses = (Resolve-DnsName $url -ErrorAction stop).IP4Address }
		catch { $ErrorMesage = $_; Write-Log -LogFile $Logfile -LogLevel ERROR -Message "Unable to resolve host $($url)." -ConsoleOutput; Write-Log -LogFile $Logfile -LogLevel ERROR -Message $($ErrorMessage); Continue }
	...
	If ($AdditionalResources)
	{
		...
		foreach ($url in $AdditionalResources)
		{
			if ($url -match "\:")
			{
				...
				catch { $ErrorMessage = $_; Write-Log -LogFile $Logfile -LogLevel ERROR -Message "Unable to resolve host $($Name)." -ConsoleOutput; Write-Log -LogFile $Logfile -LogLevel ERROR -Message $($ErrorMessage); Continue }
...
function SystemConfiguration
{
	...
	{
		"10.0"	{
					Write-Log -Logfile $Logfile -ConsoleOutput -LogLevel INFO -Message "Checking TLS settings for Windows Server 2016."
					$KeysArray = @(
